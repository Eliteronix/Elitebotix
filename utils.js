	...
	async getOsuBeatmap(input) {
		...
		for (let i = 0; i < 3; i++) {
			if (!dbBeatmap) {
				...
				if (!err.stack.includes('utils.js:3840:') && !err.stack.includes('osu-matchtrack') && !err.stack.includes('multiToBanchoScore') && !err.stack.includes('osu-top.js:956:') && !err.stack.includes('osu-score.js')) {
