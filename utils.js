...
module.exports = {
	...
	async createDuelMatch(client, bancho, interaction, averageStarRating, lowerBound, upperBound, bestOf, onlyRanked, users) {
		...
		for (let i = 0; i < users.length; i++) {
			...
			if (i < teamSize) {
				team1.push(users[i]);
				teamname1 += users[i].osuName.substring(Math.floor(users[i].osuName.length / teamSize * perTeamIterator), Math.floor(users[i].osuName.length / teamSize) + 1);
			} else {
				team2.push(users[i]);
				teamname2 += users[i].osuName.substring(Math.floor(users[i].osuName.length / teamSize * perTeamIterator), Math.floor(users[i].osuName.length / teamSize) + 1);
			}
		}
