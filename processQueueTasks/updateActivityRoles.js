			...
			try {
				...
				for (let i = 0; i < members.length; i++) {
					...
					const serverUserActivity = await DBServerUserActivity.findOne({
						...
						where: {
							userId: members[i].id, guildId: guild.id
						},
					});
							...
							try {
								...
								if (e.message === 'Missing Access' || e.message === 'Missing Permissions') {
									if (!missingPermissionsMessage) {
										...
										if (owner) {
											await owner.send(`I could not assign an activityrole to an user because I'm missing the \`Manage Roles\` permission on \`${member.guild.name}\` or because the role is higher than my highest role.`);
							...
							try {
								...
								if (e.message === 'Missing Access' || e.message === 'Missing Permissions') {
									if (!missingPermissionsMessage) {
										const owner = await member.client.users.fetch(member.guild.ownerId);
										if (owner) {
											await owner.send(`I could not remove an activityrole from an user because I'm missing the \`Manage Roles\` permission on \`${member.guild.name}\` or because the role is higher than my highest role.`);
											missingPermissionsMessage = true;
										}
									}
									return;
				...
				console.error('Error in processQueueTasks/updateActivityRoles.js', e);
			}
