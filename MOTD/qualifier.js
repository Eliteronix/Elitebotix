...
module.exports = {
	qualifier: async function (client, bancho, mappool, players) {
		...
		if (players.length < 17) {
			return knockoutLobby(client, bancho, mappool, 1, players, users, true);
		}
		...
		setTimeout(async function () {
			...
			divideIntoGroups(client, bancho, mappool, 1, players, users);
		}, 1000 * 60 * 10);
	}
};

function divideIntoGroups(client, bancho, mappool, lobbyNumber, players, users) {
	...
	knockoutLobby(client, bancho, mappool, lobbyNumber, thisLobbyPlayers, thisLobbyUsers, false);

	if (otherPlayers.length > 0) {
		divideIntoGroups(client, bancho, mappool, lobbyNumber + 1, otherPlayers, otherUsers);
	}
}
