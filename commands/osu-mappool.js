...
module.exports = {
	...
	async execute(interaction) {
		...
		if (interaction.options.getSubcommand() === 'create') {
			...
			await interaction.editReply({ content: `Mappack for \`${mappoolName.replace(/`/g, '')}\` available [here](https://www.eliteronix.de/mappack?m=${discordUser.osuUserId}-${mappoolName.replace(/`/g, '').replace(/ +/gm, '_').replace(/\/+/gm, '_')}).` });
