...
async function drawAccInfo(input, mode, mapRank) {
	...
	ctx.font = '18px comfortaa, sans-serif';
	if (score.rank !== 'F' && !score.perfect) {
		console.log(`https://osu.gatari.pw/api/v1/pp?b=${beatmap.beatmapId}&a=${accuracy}&x=0&c=${beatmap.maxCombo}&mods=${score.raw_mods}`);
		let response = await fetch(`https://osu.gatari.pw/api/v1/pp?b=${beatmap.beatmapId}&a=${accuracy}&x=0&c=${beatmap.maxCombo}&mods=${score.raw_mods}`);
