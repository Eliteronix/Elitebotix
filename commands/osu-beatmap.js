...
const { getGameMode, getIDFromPotentialOsuLink, getOsuBeatmap, getModBits, getMods, getModImage, checkModsCompatibility, getOsuPP, getScoreModpool, humanReadable, getBeatmapCover } = require('../utils');
...
async function drawStats(input, accuracy, client) {
	...
	ctx.font = 'bold 30px comfortaa, arial';
	ctx.fillText(`${Math.round(beatmap.starRating * 100) / 100}*`, canvas.width / 1000 * 330, canvas.height / 500 * 260, 220);
	...
	ctx.font = 'bold 30px comfortaa, arial';
	ctx.fillText(`${Math.round(await getOsuPP(beatmap.beatmapId, null, null, beatmap.mods, 99.00, 0, beatmap.maxCombo, client))} pp`, canvas.width / 1000 * 580, canvas.height / 500 * 440);
	...
	ctx.font = 'bold 30px comfortaa, arial';
	ctx.fillText(`${Math.round(await getOsuPP(beatmap.beatmapId, null, null, beatmap.mods, 100.00, 0, beatmap.maxCombo, client))} pp`, canvas.width / 1000 * 750, canvas.height / 500 * 440);
